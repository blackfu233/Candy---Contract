...
class CandyOrdersScene extends Phaser.Scene {
  ...
  async playSpecialComboFx(first, second, firstPos, secondPos, comboType, comboData = null) {
    ...
    const veil = this.add.rectangle(W / 2, this.boardY + this.cell * ROWS / 2, W, this.cell * ROWS + 32, 0x16091f, 0.34).setDepth(20);
    ...
    if (comboType === "stripeStripe" || comboType === "stripeBomb") {
      ...
      cols.forEach((cc, i) => {
        ...
        const beam = this.addFxRect(this.cellX(cc), this.boardY + this.cell * ROWS / 2, comboType === "stripeBomb" ? 14 : 20, this.cell * ROWS + 28, i % 2 ? 0xff8fc7 : 0x8ee8ff, 0.9).setDepth(21);
  ...
  createUi() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      this.orderRows.push({ glow, frameArt, panel, dotA, dotB, icon, label, progress, reward, iconGroup: null, nearTweens: [] });
    }
  ...
  startSession() {
    ...
    this.walletGainText.setText("").setAlpha(1).setY(656);
    this.winText.setText("");
    this.statusText.setText("Complete orders to earn instant rewards");
    this.generateOrders();
    this.generateBoard();
  ...
  setMode(mode) {
    ...
    this.walletMeterGlow.setVisible(!isBet);
    this.gameWalletText.setVisible(!isBet);
    this.walletGainText.setVisible(!isBet);
    this.gameBetText.setVisible(!isBet);
    this.winMeterGlow.setVisible(!isBet);
    this.winMeterPanel.setVisible(!isBet);
    this.stepsText.setVisible(!isBet);
    this.winGainText.setVisible(!isBet);
  ...
  createOrder(tier) {
    let spec;
    if (tier === "Easy") {
      spec = { kind: "color", type: Phaser.Utils.Array.GetRandom(TYPES), need: 8, mult: 4 };
    } else if (tier === "Medium") {
      const pool = [
        ...TYPES.map((type) => ({ kind: "color", type, need: 16, mult: 10 })),
        { kind: "any", need: 30, mult: 11 },
        { kind: "combo", comboType: "any", need: 1, mult: 14 }
      ];
      spec = Phaser.Utils.Array.GetRandom(pool);
    } else {
      const pool = [
        ...TYPES.map((type) => ({ kind: "color", type, need: 28, mult: 22 })),
        { kind: "chocolate", need: 3, mult: 28 },
        { kind: "cascade", need: 6, mult: 30 },
        { kind: "combo", comboType: "stripeStripe", need: 2, mult: 32 },
        { kind: "combo", comboType: "stripeBomb", need: 1, mult: 34 },
        { kind: "combo", comboType: "bombBomb", need: 1, mult: 36 },
        { kind: "combo", comboType: "chocolateSpecial", need: 1, mult: 38 }
      ];
      spec = Phaser.Utils.Array.GetRandom(pool);
    }
  ...
  async activateMovedSpecials(a, b, positions) {
    ...
    const addCell = (r, c) => {
      if (r >= 0 && r < ROWS && c >= 0 && c < COLS) remove.add(`${r},${c}`);
    };
    const addRow = (r) => {
    ...
    const addCol = (c) => {
      ...
      for (let r = 0; r < ROWS; r++) addCell(r, c);
    };
  ...
  recordSpecialCombo(specials) {
    ...
    this.comboCounts.any += 1;
    if (striped === 2) this.comboCounts.stripeStripe += 1;
    else if (striped === 1 && bombs === 1) this.comboCounts.stripeBomb += 1;
    else if (bombs === 2) this.comboCounts.bombBomb += 1;
    else if (chocolates >= 1) this.comboCounts.chocolateSpecial += 1;
    this.updateOrders();
  }
  ...
  async removePositions(remove, create) {
    ...
    if (this.fulfillCompletedOrders() === 0) this.updateOrders();
    if (targets.length) {
  ...
  updateOrders() {
    this.orders.forEach((order, i) => {
      ...
      row.label.setText(near ? `${order.tier.toUpperCase()}  READY SOON` : this.orderRowLabel(order));
  ...
  rawOrderProgress(order) {
    ...
    if (order.kind === "combo") return this.comboCounts[order.comboType] || 0;
    return 0;
  }
  ...
  orderRowLabel(order) {
    if (order.kind !== "combo") return `${order.tier.toUpperCase()}  x${order.need}`;
    const names = {
      any: "ANY COMBO",
    ...
    return `${order.tier.toUpperCase()}  ${names[order.comboType] || "COMBO"} x${order.need}`;
  }
  ...
  async finishMove() {
    ...
    if (this.moveReward > 0) {
      ...
      this.statusText.setText("Order complete. A new order is ready.");
