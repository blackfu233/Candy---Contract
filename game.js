...
class CandyOrdersScene extends Phaser.Scene {
  ...
  createOrder(tier) {
    let spec;
    if (tier === "Easy") {
      spec = { kind: "color", type: Phaser.Utils.Array.GetRandom(TYPES), need: 14, mult: 7 };
    } else if (tier === "Medium") {
      const pool = [
        ...TYPES.map((type) => ({ kind: "color", type, need: 32, mult: 22 })),
        { kind: "any", need: 56, mult: 26 },
        { kind: "cascade", need: 5, mult: 30 },
        { kind: "combo", comboType: "any", need: 2, mult: 34 }
      ];
      spec = Phaser.Utils.Array.GetRandom(pool);
    } else {
      const pool = [
        ...TYPES.map((type) => ({ kind: "color", type, need: 68, mult: 70 })),
        { kind: "chocolate", need: 6, mult: 86 },
        { kind: "cascade", need: 11, mult: 92 },
        { kind: "combo", comboType: "stripeStripe", need: 4, mult: 104 },
        { kind: "combo", comboType: "stripeBomb", need: 3, mult: 112 },
        { kind: "combo", comboType: "bombBomb", need: 2, mult: 124 },
        { kind: "combo", comboType: "chocolateSpecial", need: 2, mult: 138 }
      ];
      spec = Phaser.Utils.Array.GetRandom(pool);
    }
  ...
  createFreeOrder(tier) {
    const pools = {
      "Bonus Easy": [
        ...TYPES.map((type) => ({ kind: "color", type, need: 18, mult: 70 })),
        { kind: "any", need: 40, mult: 82 }
      ],
      "Bonus Medium": [
        ...TYPES.map((type) => ({ kind: "color", type, need: 34, mult: 145 })),
        { kind: "cascade", need: 4, mult: 165 },
        { kind: "combo", comboType: "any", need: 1, mult: 185 }
      ],
      "Bonus Hard": [
        ...TYPES.map((type) => ({ kind: "color", type, need: 54, mult: 290 })),
        { kind: "chocolate", need: 3, mult: 340 },
        { kind: "combo", comboType: "stripeBomb", need: 2, mult: 380 },
        { kind: "combo", comboType: "chocolateSpecial", need: 1, mult: 460 }
      ]
    };
  ...
  drawOrderIcon(row, order, x, y) {
    ...
    if (order.kind === "color") {
      ...
      keys.forEach((key, idx) => {
        const iconImg = this.add.image(x - 9 + idx * 18, y, key);
        iconImg.setScale(20 / Math.max(iconImg.width, iconImg.height));
        row.iconGroup.add(iconImg);
      });
